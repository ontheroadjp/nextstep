...
- ヘルスチェック: `live`（生存確認）と `ready`（必須環境変数確認）。
...
根拠: `app/api/_helpers.ts`, `app/api/_queries.ts`, `app/api/*/route.ts`, `app/api/health/live/route.ts`, `app/api/health/ready/route.ts`, `app/api/_utils.ts`, `app/_lib/monitoring.ts`
